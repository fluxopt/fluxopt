...
class TestEffectsTable:
        ...
        data = ModelData.build(
            ...
            ports=[
                Port(
                    id='src',
                    imports=[
                        Flow(carrier='b', size=100, effects_per_flow_hour={'cost': 0.04, 'co2': [0.2, 0.3, 0.1]}),
                    ],
                ),
                Port(id='snk', exports=[Flow(carrier='b', size=100)]),
            ],
        )
        ec = data.flows.effect_coeff
    ...
    def test_empty_when_no_flow_has_effects(self):
        ...
        data = ModelData.build(
            ...
            carriers=[Carrier(id='b')],
            effects=[Effect(id='cost')],
            ports=[Port(id='src', imports=[Flow(carrier='b', size=100)])],
        )
