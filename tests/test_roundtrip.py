...
def _elements() -> list[object]:
    return [
        Carrier('elec', unit='MWh'),
        Effect('co2', unit='kg', total_max=1000.0, contribution_from={'cost': 0.05}),
        Flow('gas', effects_per_flow_hour={'cost': 0.04}),
        Sizing(0.0, 100.0, effects_per_size={'cost': 50.0}),
        Investment(0.0, 100.0, lifetime=20, effects_fixed_at_build={'cost': 5.0}),
        Status(uptime_min=3.0, effects_per_startup={'cost': 10.0}),
        Storage('bat', Flow('elec'), Flow('elec'), capacity=Sizing(0.0, 100.0)),
        Port('grid', imports=[Flow('elec')], exports=[Flow('elec')]),
        Converter.boiler('boiler', 0.9, Flow('gas'), Flow('heat')),
        Converter.chp('chp', 0.4, 0.45, Flow('gas'), Flow('elec'), Flow('heat')),
    ]
...
class TestRoundTrip:
    ...
    def test_component_idlist_serializes_to_list(self) -> None:
        d = to_dict(Converter.boiler('b', 0.9, Flow('gas'), Flow('heat')))
        assert isinstance(d['inputs'], list)
        assert isinstance(d['outputs'], list)

    def test_component_requalifies_flows_on_rebuild(self) -> None:
        conv = from_dict(Converter, to_dict(Converter.boiler('b', 0.9, Flow('gas'), Flow('heat'))))
    ...
    def test_sizing_investment_union_disambiguates(self) -> None:
        f = from_dict(Flow, to_dict(Flow('g', size=Investment(0.0, 100.0, lifetime=20))))
...
class TestProfileRef:
    def test_profileref_roundtrips_as_variate(self) -> None:
        f = Flow('gas', effects_per_flow_hour={'cost': ProfileRef(source='prices', variable='gas')})
        d = to_dict(f)
